...
## Overview
고효율의 비즈니스 생산성을 지원하는 직관적이고 군더더기 없는 데이터 밀도 중심의 인터페이스를 지향한다.
...
## Typography
- 데이터 테이블 및 폼 레이블은 가독성을 위해 14~16px 기준 고대비(WCAG AA 4.5:1 이상)를 유지한다.
- 시각적 위계를 위해 헤딩과 본문, 캡션 간의 명확한 서체 크기 및 굵기 대비를 유지한다.

## Components
버튼과 입력 필드를 정의한다. 변형 상태는 `button-primary-hover`처럼 관련 키로 분리한다.

## Do's and Don'ts
- **Do**: 모든 핵심 CTA 버튼은 명확한 레이블과 44px 이상의 인터랙션 영역을 유지한다.
- **Don't**: 상태 표시 색상을 브랜드 장식용 배경으로 남용하지 않는다.
```
...
### 관리 및 동기화 흐름
1. **Web UI 관리**: 웹 브라우저(`/admin`)에서 디자인 시스템별 `DESIGN.md` 마크다운과 토큰을 시각적으로 편집 및 생성한다. 저장은 DB에 반영되는 동시에 `DESIGN.md` 파일로 **즉시 write-through** 되어 Git 변경 사항으로 노출된다. write-through는 보존된 Front Matter 원문(`design_systems.front_matter_raw`)을 기반으로 **대상 토큰만 in-place 치환**하므로, 주석·공백·따옴표 스타일·키 순서가 온전히 보존되고 단일 토큰 변경 시 `git diff` 1줄 변경으로 국소화된다.
2. **에이전트 컨텍스트 제공**: 에이전트가 `design://systems/{id}` 리소스를 요청하면 DB에 저장된 마크다운 가이드와 토큰을 즉시 반환하여 프롬프트 컨텍스트에 주입한다.
3. **파일 → DB 상시 선검사**: `git pull`, 브랜치 전환, 파일 직접 수정 등 외부 변경 가능성에 대응하여, 매 도구 호출 시 `(mtime_ns, size)`를 선검사하고 불일치 시에만 해시를 비교해 자동 재인덱싱한다.
