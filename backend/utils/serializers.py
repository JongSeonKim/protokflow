...
def select_columns_serialize[R: RowData](row: R) -> dict[str, Any]:
    """
    Serialize SQLAlchemy query table columns, excluding relationship columns
...
def select_as_dict[R: RowData](row: R, *, use_alias: bool = False) -> dict[str, Any]:
    """
    Convert SQLAlchemy query result to dictionary, optionally including relationship data
...
def select_join_serialize[R: RowData](  # ruff:ignore[complex-structure]
    ...
    *,
    ...
    """
    Serialize SQLAlchemy join query results into dictionaries or namedtuples

    Flat serialization (relationships=None):
        All results are flattened to the same level, no nesting
        e.g.: Result(name='Alice', dept=Dept(...))

    Nested serialization:
        Organize data hierarchically based on relationship types
        e.g.: relationships=['User-m2o-Dept', 'User-m2m-Role:permissions']
    ...
    Relationship format: source_model-type-target_model[:custom_name]
        ...
        - o2m/m2m: target field name is automatically pluralized with 's'
        - m2o/o2o: target field name remains singular
        - custom_name: custom target field name

    :param row: SQLAlchemy query result
    :param relationships: Relationship definition list
    :param return_as_dict: True returns dict, False returns namedtuple
    :return:
    """
    ...
    # namedtuple type pre-generation
    namedtuple_cache = {}
    ...
    # Nested relationship hierarchy (built in one pass)
    hierarchy: defaultdict[
        int | str, defaultdict[str, defaultdict[int | str, list[Any]]]
    ...
    for row_item in rows_list:
        ...
        if not return_as_dict:
            result_type = namedtuple("Result", list(result_data.keys()))  # type: ignore[misc]  # ruff:ignore[collections-named-tuple]
