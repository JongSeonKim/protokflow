...
- KTD8. **내보내기 commit의 author·committer는 명시 주입하고, Git 설정이 없으면 런타임 고정 신원으로 폴백한다.** 상위 계획 Assumptions가 정한 정책("author와 committer는 리포지토리의 Git 설정을 따르며, 설정이 없으면 런타임 고정 신원을 사용한다")을 이행한다. 리포지토리 Git 설정에서 신원을 해석하고, 없으면 고정 신원 `Protokflow Runtime <runtime@protokflow.invalid>`를 사용한다. 해석한 값은 `GIT_AUTHOR_NAME`·`GIT_AUTHOR_EMAIL`·`GIT_COMMITTER_NAME`·`GIT_COMMITTER_EMAIL`로 자식 프로세스에 명시 주입하여 ambient 환경 의존을 제거한다. `commit-tree`는 hook을 실행하지 않고 `commit.gpgsign`을 존중하지 않으므로 상위 계획의 hook·서명 조건은 별도 조치 없이 충족된다. 기각한 대안: 현행처럼 ambient Git 설정에만 의존 — 신원이 설정되지 않은 서비스 계정에서 내보내기가 `unable to auto-detect email address`로 실패한다. (`Governs R19`) (`session-settled: user-approved`)
- KTD9. **plumbing 계층은 frozen dataclass가 데이터를, 모듈 함수가 동작을 담당한다.** `GitRepo(worktree_root, git_executable)` frozen dataclass를 도입하여 모든 plumbing 함수의 첫 인자로 전달한다. `IsolatedIndex`는 임시 index 경로라는 실제 상태를 보유하므로 유지하되 `GitRepo`를 품는다. 이는 `core/design_md.py`, `git/process.py`, `git/context.py`가 이미 채택한 "모듈 함수 + frozen dataclass" 형태와 일치하며, U9 내보내기 시퀀스의 6단계 연쇄에서 `worktree_root`·`git_executable` 반복 전달을 제거한다. 본 계획이 패턴 근거로 참조한 `backend/app/protokflow/storage/`는 현재 `__init__.py`만 존재하여 선례로 사용할 수 없다. 기각한 대안: 전면 객체지향(`RepoPlumbing`에 모든 메서드) — 내부 일관성을 위해 `context.py`·`process.py`까지 함께 바꿔야 한다. 두 seam 공존 유지 — U2~U13이 각기 다른 호출 규약을 선택하게 되어 이후 회수 비용이 커진다. (`Governs R19`) (`session-settled: user-approved`)
